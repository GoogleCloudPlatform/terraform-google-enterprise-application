...
variable "applications" {
  ...
  - **admin_project_id** (Optional): Admin project associated with the microservice. This hosts microservice specific CI/CD pipelines. If set, `create_admin_project_id` must be `false`.
  - **create_infra_project_id** (Required): Indicates whether an infrastructure project should be created for the microservice (one infra project will be created per environment defines in var.envs).
  - **create_admin_project_id** (Required): Indicates whether a Admin project should be created for the microservice.
  EOF
  type = map(map(object({
    admin_project_id        = optional(string, null)
    create_infra_project_id    = bool
    create_admin_project_id_id = bool
  })))
      ...
      [
        for app_name, microservices in var.applications : alltrue(
          [
            ...
            (microservice_obj.admin_project_id == null || microservice_obj.create_admin_project_id == false)
          ]
        )
      ]
    )
    error_message = "If admin_project_id is specified, the corresponding create_admin_project_id must be set to false."
  }
}
