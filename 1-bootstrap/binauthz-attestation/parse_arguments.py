...
if not attestor_regex.search(args.attestor):
    parser.error('Attestor "{attestor}" is not a valid '
                 'attestor name'.format(attestor=args.attestor))

# Enforce mutual exclusion of key flag types.
keyversion_args = [
    args.keyversion,
    args.keyversion_key,
    args.keyversion_keyring,
    args.keyversion_location,
    args.keyversion_project
]

if args.pgp_key_fingerprint and any(keyversion_args):
    parser.error('You cannot set --pgp-key-fingerprint and '
                 '--keyversion related options at the same time.')
...
if args.keyversion is not None and '/' not in args.keyversion:
    if not all(keyversion_args):
        parser.error(
            'The --keyversion-key, --keyversion-keyring, '
            '--keyversion-location, and --keyversion-project options '
            'are required if --keyversion is not a fully qualified '
            'KMS key resource identifier.'
        )
    else:
        args.keyversion = (
            'projects/{project}/locations/{location}/keyRings/{keyRing}/'
            'cryptoKeys/{cryptoKey}/cryptoKeyVersions/{keyversion}'
        ).format(
...
keyversion_regex = re.compile(
    r'^projects/[a-z0-9-]*/locations/[a-z0-9-]*/keyRings/[a-zA-Z0-9-_]*/'
    r'cryptoKeys/[a-zA-Z0-9-_]*/cryptoKeyVersions/[1-9][0-9]*$'
)

if (args.keyversion is not None and not
        keyversion_regex.search(args.keyversion)):
    parser.error(
        '"{}" is not a valid fully qualified KMS key identifier.'.format(
            args.keyversion)
        )
