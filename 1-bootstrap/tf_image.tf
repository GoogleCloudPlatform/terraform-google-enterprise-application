...
module "build_terraform_image" {
  ...
  create_cmd_body       = "echo test list perm ; gcloud storage buckets list --project=${var.project_id} && echo test copy perm ; gsutil cp ./Dockerfile ${google_storage_bucket.build_logs.url} && echo test read perm ; gsutil cat ${google_storage_bucket.build_logs.url}/Dockerfile && gsutil cp ./Dockerfile ${google_storage_bucket.simulate.url} && echo simulate builder reading file ; gcloud storage cat ${google_storage_bucket.simulate.url}/Dockerfile --impersonate-service-account=${google_service_account.builder.email} && gcloud builds submit --tag ${var.location}-docker.pkg.dev/${var.project_id}/${google_artifact_registry_repository.tf_image.name}/terraform:${local.docker_tag_version_terraform} --project=${var.project_id} --service-account=${google_service_account.builder.id} --gcs-log-dir=${google_storage_bucket.build_logs.url} || sleep 30 && gcloud storage buckets list --project=${var.project_id} && gcloud builds submit --tag ${var.location}-docker.pkg.dev/${var.project_id}/${google_artifact_registry_repository.tf_image.name}/terraform:${local.docker_tag_version_terraform} --project=${var.project_id} --service-account=${google_service_account.builder.id} --gcs-log-dir=${google_storage_bucket.build_logs.url}"
