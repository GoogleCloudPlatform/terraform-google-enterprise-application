...
locals {
  ...
  gpu_t4_zones = {
    for r_idx, r in data.google_compute_zones.available : r_idx => [
      for z in r.names : z if length([
        for mt in data.google_compute_machine_types.t4[z].machine_types : mt
        if length([for acc in mt.accelerators : acc if acc.guest_accelerator_type == "nvidia-tesla-t4"]) > 0
      ]) > 0
    ]
  }
...
data "google_compute_machine_types" "t4" {
