...
resource "local_file" "credentials" {
  ...
  filename = "${path.module}/terraform-google-credentials.json"
}
...
module "cli" {
  ...
  create_cmd_body  = "access-context-manager policies list --organization ${var.org_id} --format='value(name)'  > acm_id.txt"
  destroy_cmd_body = "access-context-manager policies list --organization ${var.org_id} --format='value(name)'  > acm_id.txt"

  service_account_key_file = local_file.credentials.filename
  activate_service_account = true
}

data "local_file" "policy-org" {
  filename = "acm_id.txt"

  depends_on = [module.cli.wait]
}


module "access_level_members" {
  source = "terraform-google-modules/vpc-service-controls/google//modules/access_level"
  policy = trim(data.local_file.policy-org.content, "\n")
  ...
  egress_policies = [
    {
      ...
      to = {
        resources = ["projects/382552220525", "projects/213331819513"], //service networking projects
        operations = {
...
resource "google_access_context_manager_service_perimeter_resource" "service_perimeter_resource" {
  perimeter_name = "accessPolicies/${trim(data.local_file.policy-org.content, "\n")}/servicePerimeters/${module.regular_service_perimeter.perimeter_name}"
  resource       = "projects/${local.project_number}"
}
