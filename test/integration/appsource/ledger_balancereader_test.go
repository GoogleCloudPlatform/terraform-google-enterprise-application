...
func TestAppSourceBalancereader(t *testing.T) {
	...
	projectID := gjson.Parse(terraform.OutputJson(t, appFactory.GetTFOptions(), "app-group")).Get("balancereader").Get("app_admin_project_id").String()
	...
	frontend := tft.NewTFBlueprintTest(t,
		tft.WithTFDir("../../../5-appinfra/apps/ledger-balancereader/envs/shared"),
		tft.WithVars(vars),
	...
	frontend.DefineVerify(func(assert *assert.Assertions) {
		...
		appRepo := fmt.Sprintf("https://source.developers.google.com/p/%s/r/eab-cymbal-bank-ledger-balancereader", projectID)
		region := "us-central1"
		pipelineName := "balancereader"
		prodTarget := "dev"
		...
		err = cp.Copy("../../../6-appsource/cymbal-bank/ledger-balancereader/skaffold.yaml", fmt.Sprintf("%s/src/ledger/balancereader/skaffold.yaml", tmpDirApp))
		if err != nil {
			t.Fatal(err)
		}
