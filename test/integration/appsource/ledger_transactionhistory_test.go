...
func TestAppSourceTransactionHistory(t *testing.T) {
	...
	projectID := gjson.Parse(terraform.OutputJson(t, appFactory.GetTFOptions(), "app-group")).Get("transactionhistory").Get("app_admin_project_id").String()
	...
	frontend.DefineVerify(func(assert *assert.Assertions) {
		...
		appRepo := fmt.Sprintf("https://source.developers.google.com/p/%s/r/eab-cymbal-bank-ledger-transactionhistory", projectID)
		region := "us-central1"
		pipelineName := "transactionhistory"
		prodTarget := "dev"
		...
		err = cp.Copy("../../../6-appsource/cymbal-bank/ledger-transactionhistory/skaffold.yaml", fmt.Sprintf("%s/src/ledger/transactionhistory/skaffold.yaml", tmpDirApp))
		if err != nil {
			t.Fatal(err)
		}
