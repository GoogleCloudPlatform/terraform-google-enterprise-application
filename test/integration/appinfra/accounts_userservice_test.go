...
func TestAppinfraUserservice(t *testing.T) {
	...
	frontend := tft.NewTFBlueprintTest(t,
		tft.WithTFDir(fmt.Sprintf("../../../5-appinfra/apps/accounts-userservice/envs/shared")),
		tft.WithVars(vars),
	...
	frontend.DefineVerify(func(assert *assert.Assertions) {
		...
		appRepo := fmt.Sprintf("https://source.developers.google.com/p/%s/r/eab-cymbal-bank-accounts-userservice", projectID)
		region := "us-central1"
		pipelineName := "userservice"
		...
		err = cp.Copy("../../../6-appsource/cymbal-bank/accounts-userservice/skaffold.yaml", fmt.Sprintf("%s/src/accounts/userservice/skaffold.yaml", tmpDirApp))
		if err != nil {
			t.Fatal(err)
		}
		err = cp.Copy("../../../6-appsource/cymbal-bank/accounts-db/accounts-db.yaml", fmt.Sprintf("%s/src/accounts/accounts-db/k8s/overlays/development/accounts-db.yaml", tmpDirApp))
		if err != nil {
			t.Fatal(err)
		}
