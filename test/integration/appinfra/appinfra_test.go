...
func TestAppInfra(t *testing.T) {
	...
	for appName, serviceNames := range testutils.ServicesNames {
		appName := appName
		appSourcePath := fmt.Sprintf("../../../5-appinfra/apps/%s", appName)
				...
				appService.DefineVerify(func(assert *assert.Assertions) {
					...
					for _, v := range env_cluster_membership_ids {
						for _, cluster_membership_id := range v["cluster_membership_ids"] {
							...
							cloudDeployTarget := cluster_membership_id[len(cluster_membership_id)-1]
							cloudDeployTargets = append(cloudDeployTargets, fmt.Sprintf("%s-%s", servicesInfoMap[fullServiceName].ServiceName, cloudDeployTarget))
						}
					}
