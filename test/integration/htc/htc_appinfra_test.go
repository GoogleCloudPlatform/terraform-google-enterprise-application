...
func TestHTCAppInfra(t *testing.T) {
	...
	infraPath := fmt.Sprintf("%s/%s/envs/production", appSourcePath, appName)
	serviceAccount := strings.Split(appFactory.GetJsonOutput("app-group").Get(fmt.Sprintf("%s\\.%s.app_cloudbuild_workspace_cloudbuild_sa_email", appName, appSourcePath)).String(), "/")
