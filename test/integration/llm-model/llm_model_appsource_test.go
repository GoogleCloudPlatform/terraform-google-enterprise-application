...
func TestSourceLLMModel(t *testing.T) {
	...
	t.Run("replace-repo-contents-and-push", func(t *testing.T) {
		...
		appsource.DefineVerify(func(assert *assert.Assertions) {
			...
			utils.Poll(t, testutils.PollCloudBuild(t, buildListCmd, region, serviceName, onRetryBuild), 80, 60*time.Second)
			...
			utils.Poll(t, testutils.PollCloudDeployRelease(t, releaseListCmd, &releaseName), 80, 60*time.Second)
