...
| private\_worker\_pool | The Private Worker Pool id to be used by Cloud Build. If not provided, a new one will be created and workerpool\_network\_id must be provided. | <pre>object({<br>    private_worker_pool_id = optional(string)<br>    workerpool_network_id  = optional(string)<br>  })</pre> | n/a | yes |
