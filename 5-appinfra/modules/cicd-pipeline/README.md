...
| attestation\_kms\_key | The KMS Key ID to be used by attestor in format projects/PROJECT\_ID/locations/KMS\_KEY\_LOCATION/keyRings/KMS\_KEYRING\_NAME/cryptoKeys/KMS\_KEY\_NAME/cryptoKeyVersions/KMS\_KEY\_VERSION. | `string` | `""` | no |
| attestor\_id | The attestor name in format projects/PROJECT\_ID/attestors/ATTESTOR\_NAME. | `string` | `""` | no |
| binary\_authorization\_image | The Binary Authorization image to be used to create attestation. | `string` | `""` | no |
| binary\_authorization\_repository\_id | The Binary Authorization artifact registry where the image to be used to create attestation is stored with format `projects/{{project}}/locations/{{location}}/repositories/{{repository_id}}`. | `string` | `""` | no |
